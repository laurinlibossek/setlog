...
function LogDialog({ def, close }) {
  ...
  return html`<form class="dialog" onSubmit=${save}>
    ...
    <div class="btns two">
      ...
      <button type="submit" class="btn btn-primary" id="m-save">Save</button>
    </div>
  </form>`;
}
...
function Row({ def }) {
  ...
  if (last) {
    ...
    if (ref) {
      ...
      sub += ` · ${diff > 0 ? '+' : diff < 0 ? '−' : '±'}${fmtNum(Math.abs(diff), 1)} ${unitOf(def)} in 4 weeks`;
    }
  }
...
export function MeasureTab() {
  ...
  return html`<${Screen} large="Measure">
    <button class="btn btn-primary btn-block btn-lg" id="log-bodyweight" onClick=${() => logMeasurement(MEASUREMENT_BY_ID.bodyweight)}>
      <${Icon} name="scale" />Log body weight</button>
    <div class="section"><span class="section-title">Body</span></div>
    <div class="group">${core.map((d) => html`<${Row} key=${d.id} def=${d} />`)}</div>
    <div class="section"><span class="section-title">Circumference</span></div>
    <div class="group">${body.map((d) => html`<${Row} key=${d.id} def=${d} />`)}</div>
  <//>`;
}

const RANGES = [{ value: 30, label: '1M' }, { value: 90, label: '3M' }, { value: 365, label: '1Y' }, { value: 0, label: 'All' }];
...
export function MeasureDetailScreen({ type }) {
  ...
  const rowMenu = async (m) => {
    const c = await actionSheet({ title: `${fmtVal(def, m.value)} · ${fmt.short(m.at)}`, actions: [{ label: 'Delete entry', value: 'del', destructive: true }] });
    if (c === 'del') {
      deleteMeasurement(m.id);
      toast('Entry deleted', { action: { label: 'Undo', fn: () => addMeasurement({ type: m.type, value: m.value, at: m.at }) } });
    }
  };
  const fv = (v) => `${fmtNum(v, def.kind === 'kcal' ? 0 : 1)} ${unitOf(def)}`;
  return html`<${Screen} nav=${{ back: '', title: def.label, right: html`<button class="nav-btn" onClick=${() => logMeasurement(def)} aria-label="Add entry"><${Icon} name="plus" /></button>` }}>
    <h1 class="large-title" style="font-size:28px">${def.label}</h1>
    <div class="card">
      <div class="card-title">${list[0] ? fmtVal(def, list[0].value) : '—'}</div>
      <div class="card-sub">${list[0] ? `Latest · ${fmt.short(list[0].at)}` : 'Log your first entry to start the chart'}</div>
      <${LineChart} points=${pts} fmtVal=${fv} fmtAxis=${(v, dec) => fmtNum(v, dec)} label=${def.label}
        emptyText=${list.length ? 'No entries in this range' : 'No entries yet'} />
      <div style="margin-top:10px"><${Seg} value=${range} onChange=${setRange} options=${RANGES} /></div>
    </div>
    <button class="btn btn-tinted btn-block" style="margin-top:12px" onClick=${() => logMeasurement(def)}><${Icon} name="plus" />Add entry</button>
    ${list.length > 0 && html`<div class="section"><span class="section-title">History</span></div>
      ...
      ${list.length > limit && html`<button class="btn btn-tinted btn-block" style="margin-top:10px" onClick=${() => setLimit(limit + 60)}>Show more</button>`}`}
  <//>`;
}
